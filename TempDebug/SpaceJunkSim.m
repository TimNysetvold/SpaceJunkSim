...
%     startNumLasers = 3;
    ...
    startNumJunkSatellites = 10000*.1;
    startNumJunkClouds = 300000/100*.1;    %assume 100 small objects per cloud
    startNumSatellites = 2000*.1;
    ...
    laserDistance = 1000000;         %The maximum field of view of a laser; 100km
...
for iteration=1:totalIterations
    ...
    objectsToLookAt = find(nextLookVec<=0);
    ...
    for iterator=1:length(objectsToLookAt)
        ...
        if thisWait <=0
            ...
            thisDistances(thisDistances==0) = 1e8;
            if thisType == 1
            ...
            elseif thisType ==5
                ...
                if ~isempty(junkIndexes)
                    ...
                    junkIndexes = junkIndexes(junkObjects(:,8)<=2);
                    if ~isempty(junkIndexes)
                        [spaceArray,laseredDebris] = laser([selfIndex,junkIndexes],spaceArray,laseredDebris);
                        %disp 'laseredDebris'
                    end
                end
            end
            ...
            if thisType ~= 5
                spaceArray(selfIndex,9) = round(abs(floor(min(thisDistances)/(timestep*maxVel)))); %FixMe: Not sure if this is precisely right
            end    
        end
    end
...
%     end
    ...
    %%Record data once every hour
    if mod(iteration,72000) == 0
        tempSpaceArray = spaceArray;
        tempSpaceArray(tempSpaceArray>1e10) = 0;
        tempSpaceArray = tempSpaceArray(any(tempSpaceArray,2),:);
        spaceArray = spaceArray(any(spaceArray,2),:);

        [endObjectDensity,endRiskToISS,endYearRisk,endSmallObjects,endLargeObjects,endSatRisk,sumEndSatRisk] = findStats(tempSpaceArray);

        data = [iteration,endObjectDensity,endRiskToISS',endYearRisk,endSmallObjects,endLargeObjects,endSatRisk',sumEndSatRisk,largeObjectCollisions,cloudCollisions,avoidanceActions,cleanedDebris,laseredDebris];
