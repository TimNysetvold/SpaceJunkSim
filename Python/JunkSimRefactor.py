...
for x in range(0,startNumLasers):
    ...
    laserArray = np.append(laserArray,creatorObject)
...
for iteration in range(0,10):
    ...
    positionArraySize = spaceArray.size+laserArray.size
    ...
    if iteration%majortimestep == 0:
        ...
        for element in watchedIndexes:
            object1 = spaceArray[element[0]]
            object2 = spaceArray[element[1]]
            position1 = object1.position
            position2 = object2.position
            distancesArray[element[0],element[1]] = np.linalg.norm(position1-position2)
    ...
    for x in laserArray:
        if x.cooldown == 0 :
            closeToThisLaserIndices = np.where(collisionsIndexes[:,1] == index)
    ...
    print(deletionIndices)
    watchedIndexesToDelete = np.full([1,2],1e9)
    spaceArray = np.delete(spaceArray,deletionIndices.astype(int))
    spaceArray = np.append(spaceArray,creatorObjectArray)
    #We must separately delete distances, because they may not be updated immediately
    distancesArray = np.delete(distancesArray,deletionIndices,0)
    distancesArray = np.delete(distancesArray,deletionIndices,1)
    
    #We must delete distances from the indexes that we're watching, or we'll throw off the indexing
    subtractor = np.zeros(watchedIndexes.shape)
    end = int(watchedIndexes.shape[0])
    for element in deletionIndices:
        mask =  np.asarray(np.where(watchedIndexes == element)).T
        watchedIndexesToDelete = np.append(watchedIndexesToDelete,mask,axis=0)
        
    
    
    watchedIndexesToDelete = np.delete(watchedIndexesToDelete,0,axis=0)
    watchedIndexesToDelete = watchedIndexesToDelete[:,0]
    watchedIndexesToDelete = np.unique(watchedIndexesToDelete)
    
    
    for element in watchedIndexesToDelete:
        start = int(element)
        subtractor[start:end,:] += 1
    
    watchedIndexes = watchedIndexes - subtractor 
        
    watchedIndexes = np.delete(watchedIndexes,watchedIndexesToDelete[:,0],axis=0)
