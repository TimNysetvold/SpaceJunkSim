...
def val_equal(matrix,criteria):
    ...
    for item in matrix:
        if any(item == criteria):
            ...
            for item2 in item:
                if item2 == criteria:
                    output = np.append(np.copy(output),[[index1,index2]])
                else:
                    index2 +=1
        index1 +=1
    ...
    for item in matrix:
        if any(item <= criteria):
            if isinstance(item, int):
                ...
                colindices = [i for i, x in enumerate(item) if x <= criteria]
                colindices = np.asarray(colindices)
                for index2 in colindices:
                    output = np.append(np.copy(output),[[index1,index2]])
        index1 +=1
...
for iteration in range(0,5):
    ...
    watchedIndexes = np.asarray(val_less(distancesArray,watchDistance))
    watchedMatrix = spaceArray[watchedIndexes]
    collisionsIndexes = np.asarray(val_less(distancesArray,collisionDistance))
    cleanablesIndexes = np.asarray(val_less(distancesArray,cleanerDistance))
    lasersIndexes = np.asarray(val_less(distancesArray,laserDistance))
    watchedIndexes = np.asarray(val_less(distancesArray,watchDistance))
    ...
    for currentObject in spaceArray:
        ...
        if objectType == "CleanerSatellite":
            ...
            closeToThisCleanerIndices = val_equal(cleanablesIndexes[:,1],index)
            ...
            if currentObject.cooldown == 0 :
                closeToThisLaserIndices = val_equal(lasersIndexes[:,1],index)
            ...
            closeToThisJunkIndices = val_equal(collisionsIndexes[:,1],index)
